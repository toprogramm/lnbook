...
короткий ID канала(short channel ID - scid)::
    После установления канала индекс транзакции внесения в блокчейне используется в качестве короткого идентификатора канала для его уникальной идентификации.
    ...
    Второе число (2 байта) - это индекс транзакции внесения с блоками.
...
транзакция (transaction)::
    ...
    Первая транзакция в каждом блоке, называемая транзакцией coinbase, генерирует новый биткойн.
