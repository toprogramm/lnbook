...
Добро пожаловать в _Освоение Lightning Network_!(((Welcome to _Mastering the Lightning Network_!)))
...
=== Основные Концепты Lightning Network(((Lightning Network Basic Concepts)))
...
=== Доверие в Децентрализованных Сетях(((Trust in Decentralized Networks)))
...
=== Справедливость Без Центрального Управления(((Fairness Without Central Authority)))
...
==== Доверенные Протоколы Без Посредников(((Trusted Protocols Without Intermediaries)))
...
((("game theory")))Использование стимулов и ограничений для достижения справедливых результатов является одним из аспектов раздела математики, называемого _теорией игр_, который изучает "модели стратегического взаимодействия между рационально мыслящими людьми". footnote:[Википедия (на английском) https://en.wikipedia.org/wiki/Game_theory[статья по теории игр] предоставляет дополнительную информацию.] footnote:[Википедия (на русском) https://ru.wikipedia.org/wiki/%D0%A2%D0%B5%D0%BE%D1%80%D0%B8%D1%8F_%D0%B8%D0%B3%D1%80[статья по теории игр] предоставляет дополнительную информацию.] Криптографические системы, которые контролируют финансовые взаимодействия между участниками, такие как Bitcoin и Lightning Network, в значительной степени полагаются на теорию игр, чтобы препятствовать мошенничеству участников и позволить участникам, которые не доверяют друг другу достигать справедливых результатов.
...
==== Протокол Справедливости в Действии(((A Fairness Protocol in Action)))
...
==== Примитивы Безопасности как Строительные Блоки(((Security Primitives as Building Blocks)))
...
==== Пример Протокола Справедливости(((Example of the Fairness Protocol)))

((("fairness protocol","Proof of Work example")))((("PoW (Proof of Work) algorithm")))((("Proof of Work (PoW) algorithm")))
Наиболее ярким примером протокола справедливости является консенсусный алгоритм Bitcoin «Доказательство работы» (PoW - Proof of Work). В Bitcoin майнеры конкурируют за проверку транзакций и объединение их в блоки. Чтобы гарантировать, что майнеры не обманывают, не давая им полномочий, Bitcoin использует систему стимулов и сдерживаний. Майнерам нужно использовать электричество и выделять оборудование для выполнения "работы", которая заложена в качестве "доказательства" внутри каждого блока. Это достигается благодаря свойству хеш-функций, при котором выходное значение случайным образом распределяется по всему диапазону возможных выходных данных. Если майнерам удается достаточно быстро создать действительный блок, они получают вознаграждение(block reward) за этот блок. Стумуляция майнеров использовать много электроэнергии до того, как сеть рассмотрит их блок, означает, что у них есть стимул правильно проверять транзакции в блоке. Если они обманывают или допускают какую-либо ошибку, их блок отклоняется, а электричество, которое они использовали, чтобы «доказать» его, тратится впустую. Никому не нужно заставлять майнеров производить действительные блоки; награда и наказание стимулируют их к этому. Все, что нужно протоколу, — это гарантировать, что принимаются только действительные блоки с Доказательством Работы (only valid blocks with Proof of Work).
...
Мы можем обнаружить модель протокола справедливости во многих различных аспектах Lightning Network:
(((The fairness protocol pattern can also be found in many different aspects of the Lightning Network:)))

* Те, кто вносят средства в каналы, должны убедиться, что у них есть подписанная транзакция возврата, прежде чем публиковать транзакцию внесения.
(((* Those who fund channels make sure that they have a refund transaction signed before they publish the funding transaction.)))

* Всякий раз, когда канал перемещается в новое состояние, старое состояние "отменяется", гарантируя, что если кто-либо попытается передать его (старое состояние), он потеряет весь баланс и будет наказан.
...
* Те, кто пересылает платежи, знают, что если они пересылают средства, они могут либо получить возмещение, либо получить оплату от предшествующего для них узла (node).
...
Снова и снова мы видим эту закономерность. Справедливые результаты не обеспечены какими-либо властями. Они возникают как естественное следствие протокола, который вознаграждает честность и наказывает за мошенничество, протокол справедливости, который использует личные интересы участников, направляет их на достижение справедливых результатов.
...
Биткойн и Lightning Network являются реализациями протоколов справедливости. Так зачем же нам нужна Lightning Network? Разве Bitcoin недостаточно?(((range="endofrange", startref="ix_01_introduction-asciidoc2")))
...
=== Мотивация для Lightning Network(((Motivation for the Lightning Network)))

((("Lightning Network (generally)","motivation for", id="ix_01_introduction-asciidoc3", range="startofrange")))
Биткойн — это система, которая записывает транзакции в глобально реплицируемый публичный реестр (его можно повторить и на своем компьютере). Каждая транзакция просматривается, проверяется и сохраняется на каждом участвующем компьютере. Как вы можете себе представить, это создает много данных, и такую систему трудно масштабировать.
...
По мере роста Биткойна и спроса на транзакции количество транзакций в каждом блоке увеличивалось, пока в конечном итоге не достигло размера блока.
Как только блоки "заполняются", оставшиеся транзакции ждут в очереди. Многие пользователи увеличивают комиссию, которую они готовы заплатить, чтобы купить место для своих транзакций в следующем блоке.
...
Если спрос продолжит превышать пропускную способность сети, все большее число транзакций пользователей будут оставаться в статусе ожидании - не подтверждено. Конкуренция за комиссионные также увеличивает стоимость каждой транзакции, делая многие транзакции с маленькой суммой (например, микротранзакции) совершенно неэкономичными в периоды особенно высокого спроса.

[NOTE]
====
Средняя цена транзакции в Bitcoin на 3 Ноября 2023 $1.5-2.
В моменты перегруженности сети вы можете платить бóльшую комиссию. Воображаемая ситуация, при нагруженности сети.
Например чтобы послать эквивалент $7 в биткоин вам нужно будет заплатить еще $7.2 комиссии, Итого $14,2.
====
...
Чтобы решить эту проблему, мы могли бы увеличить лимит размера блока, чтобы освободить место для большего количества транзакций. Увеличение "предложения" блочного пространства приведет к снижению цены комиссии за транзакции.
...
Однако увеличение размера блока переносит затраты на операторов узлов(node operators) и требует от них расходовать больше ресурсов на проверку и хранение блокчейна. Поскольку блокчейны — это протоколы сообщения, каждый узел должен знать и проверять каждую транзакцию, происходящую в сети. Более того, после проверки каждая транзакция и блок должны распространиться среди "соседей" узла (node neighbors), что увеличивает требования к пропускной способности. Таким образом, чем больше размер блока, тем выше требования к пропускной способности, обработке и хранению для каждого отдельного узла. Увеличение пропускной способности транзакций таким образом имеет нежелательный эффект централизации системы за счет уменьшения количества узлов и операторов узлов. Если эксплуатация узлов очень дорогая, операторы узлов не получат вознаграждения за работу узлов, и только несколько хорошо финансируемых операторов узлов продолжат эксплуатировать узлы.
...
==== Масштабирование Блокчейннов(((Scaling Blockchains)))

((("blockchain","scaling", id="ix_01_introduction-asciidoc4", range="startofrange")))((("Lightning Network (generally)","scaling blockchains", id="ix_01_introduction-asciidoc5", range="startofrange")))Побочные эффекты увеличения размера блока или уменьшения времени блока учитывая централизацию сети довольно серьезны, как показывают несколько расчетов с цифрами.
...
Предположим, что использование Bitcoin вырастет настолько, что сети придется обрабатывать 40 000 транзакций в секунду, что является приблизительным уровнем обработки транзакций в сети Visa в пиковые моменты.
...
Если предположить, что в среднем на одну транзакцию приходится 250 байт, потребуется поток данных 10 мегабайт в секунду (МБ/с) или 80 мегабит в секунду (Мбит/с) только для приема всех транзакций .
Сюда не входит дополнительнвй трафик на пересылку информации о транзакциях другим участникам сети.
Хотя скорость 10 Мбайт/с не кажется гигантской в контексте высокоскоростного оптоволокна и мобильных скоростей 5G, она фактически исключает возможность запуска и работы узла для тех, кто не может выполнить это требование, особенно в странах, где высокопроизводительный Интернет не доступен или мало распространен.
...
У пользователей также есть много других требований к пропускной способности их сети, и не слудует ожидать(((не предполагают тратить на это))), что они будут ее тратить только на получение транзакций.
...
Кроме того, локальное хранение этой информации приведет к расходу 864 гигабайт в день. Это примерно один терабайт данных или емкость домашнего жесткого диска.
...
Проверка 40 000 подписей алгоритма цифровой подписи на основе эллиптической кривой (Elliptic Curve Digital Signature Algorithm  - ECDSA) в секунду также едва ли осуществима (см https://bitcoin.stackexchange.com/questions/95339/how-many-bitcoin-transactions-can-be-verified-per-second[эту статью на английском на StackExchange ]), что делает _начальную загрузку блока (IBD)_ блокчейна Bitcoin (синхронизацию и проверку всего, начиная с генезис-блока) практически невозможной без очень дорогого оборудования.
...
Теперь когда 40 000 транзакций в секунду выглядят весомо, и зная что паритет с традиционными финансовыми платежными сетями достигается только в пиковые моменты. Инновации в платежах машина-машина, микротранзакциях и других приложениях, скорее всего подтолкнут спрос на порядки выше чем сейчас.
...
Проще говоря: вы не можете масштабировать блокчейн для проверки транзакций всего мира децентрализованно.
(((Simply put: you can't scale a blockchain to validate the entire world's transactions in a decentralized way.)))

_Но что, если бы от каждого узла не требовалось знать и проверять каждую транзакцию? Что, если бы существовал способ масштабировать off-chain транзакции без потери безопасности сети Биткойн?_
...
В феврале 2015 Джозеф Пун и Таддеус Дрия предложили возможное решение проблемы масштабируемости Bitcoin, опубликовав статью «Сеть Lightning Bitcoin-а : Масштабируемые Мгновенные Off-Chain Платежи».footnote:[Джозеф Пун и Таддеус Дрия. "Сеть Lightning Bitcoin-а : Масштабируемые Мгновенные Off-Chain Платежи" DRAFT Version 0.5.9.2. 14 Января, 2016. https://lightning.network/lightning-network-paper.pdf[] (на английском).] _("The Bitcoin Lightning Network: Scalable Off-Chain Instant Payments.")_
(((In February 2015, Joseph Poon and Thaddeus Dryja proposed a possible solution to the Bitcoin scalability problem, with the publication of "The Bitcoin Lightning Network: Scalable Off-Chain Instant Payments.")))

В (уже устаревшем) техническом документе Пун и Дрия подсчитали, что для того, чтобы Биткойн мог бы достичь 47 000 транзакций в секунду, обрабатываемых Visa на пике, ему потребуются блоки размером 8 ГБ.
Это сделает эксплуатацию узла совершенно неприемлемой для кого-либо, кроме крупных предприятий и предприятий промышленного уровня.
Результатом этого будет сеть, в которой лишь несколько пользователей смогут фактически проверить состояние реестра.
Биткойн полагается на то, что пользователи сами проверяют реестр, не доверяя третьим лицам, чтобы оставаться децентрализованным.
Цены на использование работающих узлов вынудит среднестатистического пользователя доверять третьим сторонам возможность узнать состояние реестра, что в конечном итоге нарушит модель доверия Биткойна.
...
Сеть Lightning предлагает новую сеть, второй слой, где пользователи могут совершать peer-to-peer (равный - равному, пользователь - пользователю) платежи друг другу без необходимости публикации транзакции в блокчейне Bitcoin для каждого платежа.
Пользователи могут платить друг другу в сети Lightning столько раз, сколько захотят, не создавая дополнительных биткойн-транзакций и без затрат на on-chain комиссии.
Они используют блокчейн Bitcoin только для первоначальной загрузки биткойнов в сеть Lightning и для _расчета_ (settle), то есть для удаления биткойнов из сети Lightning.
В результате гораздо больше платежей в биткойнах могут осуществляться off-chain, нужны только транзакции начальной загрузки и окончательного расчета которые проверяются и сохраняются узлами Bitcoin.
Помимо снижения нагрузки на узлы, платежи в сети Lightning обходятся дешевле для пользователей, поскольку им не нужно платить комиссию блокчейна, и более конфиденциальны для пользователей, поскольку они не публикуются для всех участников сети и, кроме того, не хранятся на постоянной основе.
...
Хотя сеть Lightning изначально была задумана для Bitcoin, ее можно реализовать на любом блокчейне, который отвечает некоторым базовым техническим требованиям. Другие блокчейны, такие как Litecoin, уже поддерживают Lightning Network. В дополнении к этому, несколько других блокчейнов разрабатывают аналогичные решения второго слоя или «слой 2», чтобы помочь им масштабироваться.(((range="endofrange", startref="ix_01_introduction-asciidoc5")))(((range="endofrange", startref="ix_01_introduction-asciidoc4"))).(((range="endofrange", startref="ix_01_introduction-asciidoc3")))
...
=== Определяющие Особенности сети Lightning(((The Lightning Network's Defining Features)))

((("Lightning Network (generally)","defining features")))Lightning Network — это сеть, которая работает как протокол второго уровня поверх Bitcoin и других блокчейнов. Сеть Lightning обеспечивает быстрые, безопасные, конфиденциальные, не требующие доверия и разрешения платежи. Вот некоторые особенности сети Lightning:
 ...
 * Пользователи сети Lightning могут отправлять платежи друг другу по низкой цене и в режиме реального времени.
 (((Users of the Lightning Network can route payments to each other for low cost and in real time.)))
 * Пользователям, которые обмениваются ценностями через сеть Lightning, не нужно ждать подтверждений блока для платежей.
 (((Users who exchange value over the Lightning Network do not need to wait for block confirmations for payments.)))
 * После завершения платежа в сети Lightning (обычно в течение нескольких секунд) он является окончательным и не может быть отменен. Как и транзакция Биткойн, платеж в сети Lightning может быть возвращен только получателю.
 ...
 * В то время как on-chain транзакции Биткойн транслируются и проверяются всеми узлами в сети, платежи, маршрутизируемые в сети Lightning, передаются между парами узлов и не видны всем, что приводит к гораздо большей конфиденциальности.
 ...
 * В отличие от транзакций в сети Bitcoin, платежи, направляемые в сеть Lightning, не требуют постоянного хранения. Таким образом, Lightning использует меньше ресурсов и, следовательно - дешевле. Это свойство также имеет преимущества в плане конфиденциальности.
 ...
 * Сеть Lightning использует луковую маршрутизацию, аналогичную протоколу, используемому в конфиденциальной сети The Onion Router (Tor), так что даже узлы, участвующие в маршрутизации платежа, знают только  о своих предшественниках и преемниках на маршруте платежа.
 ...
 * При использовании поверх Bitcoin сеть Lightning использует настоящий биткойн, который всегда находится во владении (на хранении) и под полным контролем пользователя. Lightning — это не отдельный токен или монета, _это и есть_ биткойн.
...
[[user-stories]]
=== Варианты Использования Сети Lightning, Пользователи и Их Истории(((Lightning Network Use Cases, Users, and Their Stories)))

((("Lightning Network (generally)","use cases and users")))Чтобы лучше понять, как действительно работает сеть Lightning и почему люди ее используют, мы будем наблюдать за пользователями и их историями.(((To better understand how the Lightning Network actually works, and why people use it, we'll be following a number of users and their stories.)))

В наших примерах некоторые люди уже использовали Bitcoin, а другие — совершенно новички в сети Bitcoin. Каждый человек и его история, перечисленные здесь, показывают один или несколько специфичных вариантов использования. Мы будем возвращаться к ним на протяжении всей книги:
...
Покупатель(((Consumer)))::
Алиса — пользователь Bitcoin, который хочет совершать быстрые, безопасные, дешевые и конфиденциальные платежи за небольшие розничные покупки. Она покупает кофе за биткойны, используя Сеть Lightning.
...
Продавец::
(((Merchant)))
Боб - владелец кофейни «Кафе Боба». On-chain Bitcoin платежи не подходят для небольших сумм, таких как чашка кофе, поэтому он использует сеть Lightning для почти мгновенного приема Bitcoin платежей а так же низкой комиссии.
...
Компания по обслуживанию программного обеспечения(((Software service business)))::
Чен — китайский предприниматель, занимающийся информационными сервисами, связанными с сетью Lightning, а также Bitcoin и другими криптовалютами. Чен продает услуги информационных сервисов через Интернет, осуществляя микроплатежи через сеть Lightning. Кроме того, Чен внедрил сервис по поставке ликвидности, этот сервис арендует емкость входящего канала в сети Lightning, взимая небольшую комиссию в биткойнах за каждый период аренды.
...
Геймер(((Gamer)))::
Дина — подросток-геймер из России. Она играет во множество разных компьютерных игр, но больше всего ей нравятся те, в которых есть «внутри-игровая экономика», основанная на реальных деньгах. Играя в игры, она также зарабатывает деньги, приобретая и продавая виртуальные внутри-игровые предметы. Сеть Lightning позволяет ей совершать небольшие платежи за внутри-игровые предметы, а также немного зарабатывать в криптовалюте за выполнение квестов в игре.
...
В этой главе мы говорили о фундаментальной концепции, лежащей в основе как Биткойна так же и сети Lightning: протоколе справедливости.
...
Мы рассмотрели историю сети Lightning и мотивы, лежащие в основе решений использования второго уровня для масштабирования Bitcoin и других сетей на основе блокчейна.
